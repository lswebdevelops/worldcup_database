...
echo "$($PSQL "select name from teams inner join games on (teams.team_id=games.winner_id or teams.team_id=games.opponent_id) where round='Eighth-Final' and year = 2014 order by teams.name ")"  
